...
当前版本会展示“最近一次已落盘回合”，同时通过 `/api/events` 提供 SSE live event 流。交互式 CLI 运行时会把 `context`、`llm`、`tool`、`verify`、`done`、`error`、`cancelled` 等高信号进度写入 `.ming/live/events.jsonl`，Trace Console 右侧会实时显示最近 20 条事件。当前还不是完整 Web Chat：用户输入仍从 CLI 发起，页面只负责观察运行状态；provider token 级流式输出和 Web UI 发起/停止回合属于下一阶段。
...
- Trace Console 状态聚合、HTML 渲染、空状态和 `/api/events` SSE live event。
...
2. 在另一个终端运行 `python -m ming`，输入“创建 `scratch/trace_demo.txt`，内容写入你刚才的建议，然后读取确认”。回到页面确认右侧 Live Events 会实时出现 `context`、`llm`、`tool`、`verify`、`done`，任务结束后 TODO、工具事件、最终回复和 changed files 都更新。
