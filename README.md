...
打开 `http://127.0.0.1:8765` 后，可以查看最近一轮 agent-loop 的任务、TODO、工具步骤、进展判断、可公开思路摘要、subagent 状态和 SessionTrace/checkpoint/notepad 路径。当前 UI 读取本地 `.ming/` 运行产物，不需要 API key，也不会展示底层 provider 噪音日志。

![Ming Agent Workbench 运行过程截图](docs/assets/ming-workbench-live.png)
...
CLI 默认展示高信号进度，例如“准备上下文”“调用模型”“执行工具 file_write”“执行 T3 核验”。底层 httpx、asyncio 等 provider 日志默认不刷屏；需要看详细内部日志时使用 `/debug`，需要展开每步参数时使用 `/details`，完整记录仍可通过 `/trace` 查看。

![Ming CLI 进度展示截图](docs/assets/ming-cli-progress.png)
...
错误恢复还包括 `ErrorClassifier`：区分 transient、provider、tool_input、permission 等错误，标记 retryable/recoverable，并用于后续恢复策略。LLM/provider timeout 会被转换为用户可理解的“模型服务长时间没有响应”说明，原始 provider 异常只保存在 trace、notepad 和诊断详情里，不再默认刷到对话区。
...
- 默认日志不进入 debug 模式。
- 默认压制 provider 控制台噪音，改用 agent-loop 缩略进度。
